        ...
        >
          <div class="flex">
            ...
            <el-form-item label="服务费用:" prop="expenses">
              <el-input v-model="ruleForm.expenses" placeholder="请输入服务费用"></el-input>
            </el-form-item>
            ...
            <el-form-item label="客户经理:" prop="manager1">
              <el-input placeholder="请输入客户经理工号" v-model="ruleForm.manager1"></el-input>
            </el-form-item>
            <el-form-item label="权证经理:" prop="manager2">
              <el-input placeholder="请输入权证经理工号" v-model="ruleForm.manager2"></el-input>
            </el-form-item>
            <el-form-item label="审核经理:" prop="manager2">
              <el-input placeholder="请输入审核经理工号" v-model="ruleForm.manager3"></el-input>
            </el-form-item>
            <el-form-item label="签约状态:" prop="status">
              <el-select style="width: 100%" v-model="ruleForm.status" placeholder="请选择签约状态" @change="statusChange">
      ...
      <el-table-column label="电子合同" min-width="100px">
        <template slot-scope="scope">
          <el-tooltip placement="right">
            ...
            <span
              class="colorBlue"
              @click="$common.download(scope.row.imgurl,scope.row.name)"
            >{{scope.row.name}}</span>
          </el-tooltip>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        expenses: [{ required: true, message: '请输入约定费用', trigger: 'blur' }],
        startime: [{ required: true, message: '请选择合同起始日期', trigger: 'blur' }],
        endtime: [{ required: true, message: '请选择合同结束日期', trigger: 'blur' }],
        optertime: [{ required: true, message: '请选择提示方式', trigger: 'blur' }]
      },
      tableData: [],
      tableHeader: [
        { name: '合同类型', prop: 'type' },
  ...
  methods: {
    ...
    async handSuccess(res, file, fileList) {
      if (res.code == 200) {
        if(!this.isAdd){
          await this.$axios.post(this.$api.updataContract, {_id:this.ruleForm._id,imgurl: res.data})
        }
    ...
    async editRow(index, row) {
      this.dialogVisible = true
      this.isAdd = false
      await this.$axios.post(this.$api.findOneContract, {_id:row._id}).then(res => {
        if(res.code == 200){
