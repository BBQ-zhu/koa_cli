      ...
      <span>
        ...
        >
          ...
          <el-form-item label="薪资范围:" prop="salary">
            <el-input v-model="ruleForm.salary" placeholder="请输入薪资范围"></el-input>
          </el-form-item>
          <el-form-item label="工作地址:" prop="address">
            <el-input v-model="ruleForm.address" placeholder="请输入工作地址"></el-input>
          </el-form-item>
          <el-form-item label="学历要求:" prop="education">
            <el-input v-model="ruleForm.education" placeholder="请输入学历要求"></el-input>
          </el-form-item>
        </el-form>
  ...
  methods: {
    ...
    deleteRow(index, row) {
      this.$confirm('确认删除该信息吗？', '提示', {
        ...
        this.$axios.post(this.$api.delateRecruiting, { _id: row._id }).then(res => {
          if (res.code == 200) {
      ...
      this.$refs[formName].validate(async valid => {
        if (valid) {
          ...
          delete this.ruleForm._id
            var res = await this.$axios.post(this.$api.uploadRecruiting, this.ruleForm)
          } else {
            var res = await this.$axios.post(this.$api.updataRecruiting, this.ruleForm)
          }
          if (res.code == 200) {
